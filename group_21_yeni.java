...
public class group_21 {
    ...
    public static void exitAsciiArt(){
        System.out.println("\r\n" + //
                "\r\n" + //
                "                                                                                                                                                                                                        \r\n" + //
                "                                                                                                                                                                                                        \r\n" + //
            ...
            if (input.hasNextInt()){
                ...
                case 11:
                    ...
                    try {
                        ...
                        System.out.println("An error occurred while waiting for the welcome screen to close.");
                    }
                    clearTerminal();
...
/*--------------------------------------------------------INVERSE OF A MATRIX----------------------------------------------------------------------------*/
	...
	//In order to take inverse a matrix, it has to be square!
        ...
        if (determinant == 0) {
            ...
            double[][] inverseMatrix = transposeHelper(adjointMatrix);
...
/*--------------------------------------------------------SET DIMENSIONS FOR THE MATRIX----------------------------------------------------------------------------*/
    ...
    //Set dimensions of square matrix.
    public static int[] getSquareMatrix(){
        Scanner input = new Scanner(System.in);
        int rows,columns;

        //The matrix has to be square, shouldn't consist char, and must be betwwen 2x2 and 5x5
        do{
            System.out.print("Enter the number of rows of the matrices: ");
            while (!input.hasNextInt()) {
                System.out.println("Invalid input. Please enter an integer. \n");
            ...
            if (rows != columns || !check_matrix_limits(rows, columns)) {
                System.out.println("The matrix has to be square and between 2x2 and 5x5.\n");
            }
        }while (rows != columns || !check_matrix_limits(rows, columns));
...
/*--------------------------------------------------------FILLING THE MATRIX----------------------------------------------------------------------------*/
    ...
    public static double[][] fill_matrix(int rows, int columns,double[][] matrix){
    Scanner input = new Scanner(System.in);
    //In the matrix, the cells should be existed by double values. Not chars.
        for(int i = 0; i < rows; i++){
            for(int j = 0; j < columns; j++) {
                ...
                while (!validInput) {
                    ...
                    if (input.hasNextDouble()) {
                        ...
                        System.out.println("You didn't enter a valid value. Please try again. \n");
                        input.next(); // Clean the input
                    }
                }
            }
            System.out.println();
...
/*--------------------------------------------------------HELPER FOR TAKING TRANSPOSE----------------------------------------------------------------------------*/

    public static double[][] transposeHelper(double[][] matrix) {
...
public static double[][] adjoint(){
    ...
    for(int i = 0; i < rows; i++) {
        for(int j = 0; j < columns; j++) {
            double[][] minorMatrix = subMatrix(matrix, i, j);
            int sign = (i + j) % 2 == 0 ? 1 : -1;
            adjointMatrix[j][i] = sign * calculateDeterminant(minorMatrix);
        }
    }
...
/*--------------------------------------------------------CLEAR TERMINAL----------------------------------------------------------------------------*/
    ...
    public static void main(String[] args) {
        welcomeScreen();
        try {
            int milliseconds = 2000; // Wait for 2 seconds (2000 milliseconds)
            Thread.sleep(milliseconds);
        } catch (InterruptedException e) {
            System.out.println("An error occurred while waiting for the welcome screen to close.");
        }
